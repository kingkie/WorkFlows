...
namespace Yu3zx.GoCsp
{
    ...
    public class Map<TKey, TValue>
    {
        ...
        TupleEx<bool, MapNode<TKey, TValue>> lbound_insert(TKey key)
        {
            MapNode<TKey, TValue> pNode = root;
            ...
            {
                return TupleEx.Make(true, boundWhereNode);
            }
            MapNode<TKey, TValue> newNode = new_inter_node(key, default(TValue));
            insert_at(addLeft, insertWhereNode, newNode);
            return TupleEx.Make(false, newNode);
        }

        TupleEx<bool, MapNode<TKey, TValue>> rbound_insert(TKey key)
        {
            MapNode<TKey, TValue> pNode = root;
            MapNode<TKey, TValue> insertWhereNode = _head;
            MapNode<TKey, TValue> boundWhereNode = _head;
        ...
        public TupleEx<bool, MapNode<TKey, TValue>> GetFirst(TKey key)
        {
            return lbound_insert(key);
        }

        public TupleEx<bool, MapNode<TKey, TValue>> GetLast(TKey key)
        {
        ...
        public IEnumerator<TupleEx<TKey, TValue>> GetEnumerator()
        {
            MapNode<TKey, TValue> it = First;
            while (null != it)
            {
                yield return TupleEx.Make(it.key, it.value);
                it = it.Next;
            }
            yield break;
        }

        public TupleEx<TKey, TValue>[] ToList
        {
            get
            {
                int idx = 0;
                TupleEx<TKey, TValue>[] list = new TupleEx<TKey, TValue>[_count];
                MapNode<TKey, TValue> it = First;
                while (null != it)
                {
                    list[idx++] = TupleEx.Make(it.key, it.value);
                    it = it.Next;
                }
                return list;
            }
