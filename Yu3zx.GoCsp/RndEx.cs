...
namespace Yu3zx.GoCsp
{
    ...
    public class RndEx : Random
    {
        ...
        private static readonly ThreadLocal<RndEx> _globalRandGen = new ThreadLocal<RndEx>();
        private readonly uint[] _mt = new uint[N];
        private short _mti;

        public RndEx(int seed)
        {
            Init((uint)seed);
        }
        ...
        public RndEx()
        {
            Init((uint)SystemTick.GetTick());
        }

        public RndEx(uint[] initKey)
        {
            Init(initKey);
        }

        public static RndEx Global
        {
            get
            {
                RndEx randGen = _globalRandGen.Value;
                if (null == randGen)
                {
                    randGen = new RndEx((int)SystemTick.GetTick() * Thread.CurrentThread.ManagedThreadId);
